...
CREATE TABLE listings (
  ...
  precise_location GEOMETRY(POINT, 4326), -- PostGIS (private, not shown)
  public_location VARCHAR(255), -- Approximate (shown to buyers)
  ...
  days_since_confirmed INT GENERATED ALWAYS AS (
    EXTRACT(DAY FROM NOW() - COALESCE(last_confirmed_at, created_at))
  ) STORED,
  ...
  SPATIAL INDEX idx_location (precise_location)
);
...
CREATE TABLE listing_images (
  ...
  thumbnail_url VARCHAR(255) NOT NULL, -- Wasabi CDN
  full_url VARCHAR(255) NOT NULL,      -- Wasabi CDN
...
CREATE TABLE listing_attributes (
  ...
  attribute_key VARCHAR(50), -- e.g., 'condition', 'material', 'brand'
...
CREATE TABLE messages (
  ...
  INDEX idx_listing_id_created (listing_id, created_at DESC),
...
CREATE TABLE saved_searches (
  ...
  name VARCHAR(100),
  filters JSONB, -- {category, emirate, community, price_min, price_max, etc.}
...
CREATE TABLE moderation_queue (
  ...
  reason VARCHAR(100), -- 'spam', 'prohibited', 'defect_undisclosed', etc.
  ...
  INDEX idx_status (status),
  INDEX idx_flagged_at (flagged_at DESC)
);
...
STEP 2: Database (10 min)
  ...
  - Verify tables exist: psql -l suqly_dev
...
STEP 10: Documentation (20 min)
  - Create docs/setup.md:
    ...
    * DB: psql -d suqly_dev + sample queries
